...
import idsa.connector.ConnectorException;
import onedata.onezone.OneZoneException;
import onedata.oneprovider.OneProviderException;


/**
 * An error in the operation of the DataHub connector's REST API.
 * Can be constructed manually or from exceptions.
 *
 * This is the OpenApi entity that will be returned in case of error in a DataHub connector API.
...
public class ActionError {
    ...
    public ActionError(Throwable t) {
        ...
        if (type.equals(OneZoneException.class) ||
            ...
            type.equals(ConnectorException.class) ||
            ...
            if(AbstractBuiltResponse.class.isAssignableFrom(we.getResponse().getClass()))
            {
                AbstractBuiltResponse abr = (AbstractBuiltResponse)we.getResponse();
                this.description = Optional.of(abr.getReasonPhrase());
            }
        }
